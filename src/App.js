...
export default function App() {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      alert(authMode === 'login' ? "Erro no acesso. Verifique a chave." : "Erro ao criar conta.");
    }
  };
  ...
  if (loading) return <div style={{height:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#fff', color: THEME.primary, fontFamily: THEME.fontFamily}}>Carregando Klinni IA...</div>;
      ...
      {!user ? (
        <div style={{height:'100vh', display:'flex', alignItems:'center', justifyContent:'center'}}>
          <div style={{width:'320px', padding:'45px', background:'#fff', borderRadius:'35px', boxShadow:'0 25px 60px rgba(0,0,0,0.04)', textAlign:'center'}}>
            ...
            <p style={{fontSize:'12px', color:THEME.textLight, marginBottom:'30px', fontWeight:'500'}}>
              {authMode === 'login' ? 'Bem-vindo de volta' : 'Crie sua conta profissional'}
            </p>
            
            <form onSubmit={handleAuth} style={{display:'flex', flexDirection:'column', gap:'12px'}}>
              <input placeholder="WhatsApp (DDD + Número)" value={celular} onChange={e=>setCelular(e.target.value)} required style={{padding:'16px', borderRadius:'16px', border:'1px solid #f1f5f9', outline:'none', background:'#fcfcfd'}} />
              <input type="password" placeholder="Sua Chave" value={password} onChange={e=>setPassword(e.target.value)} required style={{padding:'16px', borderRadius:'16px', border:'1px solid #f1f5f9', outline:'none', background:'#fcfcfd'}} />
              <button style={{padding:'18px', background:THEME.secondary, color:'#fff', border:'none', borderRadius:'16px', fontWeight:'700', cursor:'pointer', marginTop:'10px', boxShadow:'0 10px 20px rgba(0,0,0,0.1)'}}>
            ...
            <button 
              ...
              style={{marginTop:'25px', background:'none', border:'none', color:THEME.primary, fontSize:'13px', fontWeight:'700', cursor:'pointer'}}
            >
              {authMode === 'login' ? 'Não tem acesso? Clique aqui' : 'Já tenho conta. Fazer Login'}
            </button>
          </div>
        </div>
      ) : (
        <>
          <nav style={{display:'flex', justifyContent:'space-between', padding:'15px 50px', background:'rgba(255,255,255,0.85)', backdropFilter:'blur(12px)', borderBottom:'1px solid #f1f5f9', position:'sticky', top:0, zIndex:100}}>
            <h2 style={{fontWeight:'900', fontSize:'18px', letterSpacing:'-0.5px'}}>KLINNI <span style={{color:THEME.primary}}>IA</span></h2>
            <div style={{display:'flex', gap:'30px', alignItems:'center'}}>
              <button onClick={()=>{setView('dashboard'); resetForm();}} style={{background:'none', border:'none', cursor:'pointer', fontWeight:'600', color:view==='dashboard'?THEME.primary:THEME.textLight, transition:'0.3s'}}>Dashboard</button>
              <button onClick={()=>{setView('novoLead'); resetForm();}} style={{background:'none', border:'none', cursor:'pointer', fontWeight:'600', color:view==='novoLead'?THEME.primary:THEME.textLight, transition:'0.3s'}}>+ Novo Lead</button>
          ...
          <main style={{padding:'40px 50px', maxWidth:'1200px', margin:'0 auto'}}>
            {view === 'dashboard' && (
              <>
                {/* CALENDÁRIO COM TIPOGRAFIA SUAVE */}
                <div style={{marginBottom: '50px'}}>
                  <h3 style={{fontSize:'11px', fontWeight:'800', color:THEME.textLight, marginBottom:'18px', textTransform:'uppercase', letterSpacing:'1px'}}>Agenda da Semana</h3>
                  <div style={{display:'flex', gap:'15px', overflowX:'auto', paddingBottom:'15px'}}>
                    {getProximosDias().map((dia, idx) => {
                      ...
                      const isToday = idx === 0;
                      return (
                        <div key={idx} style={{minWidth:'170px', background:'#fff', padding:'20px', borderRadius:'24px', border: isToday ? `1px solid ${THEME.primary}33` : '1px solid #f1f5f9', boxShadow: isToday ? '0 10px 30px rgba(255,107,0,0.04)' : 'none'}}>
                          ...
                          <div style={{display:'flex', flexDirection:'column', gap:'6px'}}>
                            ...
                            )) : <div style={{fontSize:'10px', color:'#e2e8f0', fontWeight:'500'}}>Livre</div>}
                          </div>
                        </div>
                      );
                    })}
                ...
                {/* LISTAGEM DE LEADS COM DESIGN SOFT */}
                <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(300px, 1fr))', gap:'25px'}}>
                  {leads.map(l => (
                    <div key={l.id} style={{background:'#fff', padding:'30px', borderRadius:'28px', border:'1px solid #f8fafc', position:'relative', boxShadow:'0 15px 35px rgba(0,0,0,0.01)'}}>
                      ...
                      <div style={{fontSize:'9px', fontWeight:'900', color: l.categoria === 'HIGH TICKET' ? THEME.primary : '#cbd5e1', letterSpacing:'1px', marginBottom:'8px'}}>{l.categoria}</div>
                      <h4 style={{margin:'0 0 12px 0', fontWeight:'700', fontSize:'18px', color:THEME.secondary}}>{l.nome}</h4>
                      {l.dataAgendamento && <div style={{fontSize:'11px', color:'#3b82f6', fontWeight:'700', marginBottom:'15px', display:'flex', alignItems:'center', gap:'5px'}}>🗓️ {new Date(l.dataAgendamento).toLocaleString('pt-BR', {day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit'})}</div>}
                      <div style={{borderTop:'1px solid #fcfcfd', paddingTop:'15px', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                        <span style={{fontSize:'10px', color:THEME.textLight, fontWeight:'600'}}>ORÇAMENTO</span>
                        <span style={{fontSize:'17px', fontWeight:'800', color:THEME.secondary}}>R$ {formatarMoeda(l.valorOrcamento)}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {view === 'novoLead' && (
              <div style={{maxWidth:'550px', margin:'0 auto', background:'#fff', padding:'50px', borderRadius:'35px', boxShadow:'0 20px 50px rgba(0,0,0,0.03)'}}>
                <h2 style={{textAlign:'center', marginBottom:'40px', fontWeight:'900', letterSpacing:'-1px'}}>{idLeadEditando?'Refinar Lead':'Novo Lead'}</h2>
                <form onSubmit={handleSalvarLead} style={{display:'flex', flexDirection:'column', gap:'20px'}}>
                  <div style={{display:'flex', flexDirection:'column', gap:'6px'}}>
                    <label style={{fontSize:'11px', fontWeight:'700', color:THEME.textLight, marginLeft:'5px'}}>NOME DO CLIENTE</label>
                    <input placeholder="Ex: Maria Oliveira" value={nomeLead} onChange={e=>setNomeLead(e.target.value)} required style={{padding:'16px', borderRadius:'16px', border:'1px solid #f1f5f9', outline:'none', fontSize:'14px', background:'#fcfcfd'}} />
                  </div>
                  ...
                  <button type="submit" disabled={isSaving} style={{padding:'20px', background:THEME.secondary, color:'#fff', border:'none', borderRadius:'18px', fontWeight:'700', cursor:'pointer', marginTop:'15px', boxShadow:'0 10px 25px rgba(0,0,0,0.1)'}}>
                    {isSaving ? 'PROCESSANDO...' : 'CONFIRMAR DADOS'}
                  </button>
                </form>
              </div>
            )}
